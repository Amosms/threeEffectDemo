  ...
  const material = new THREE.ShaderMaterial({
    ...
    #define PI2 6.28318530718
        ...
        vec2 spaceWrap(in float rows,in float column,in vec2 _st){
            ...
            if(step(1.0,mod(uTime,2.0))>0.0){
              _st.x += (step(1.0, mod(_st.y,2.0)) * 2.0 - 1.0) *uTime;
            }else{
              _st.y += (step(1.0, mod(_st.x,2.0)) * 2.0 - 1.0) *uTime;
            }
            return fract(_st);

            // if(sin(uTime)>0.0){
            //   _st.x += (step(1.0, mod(_st.y,2.0)) * 2.0 - 1.0) * abs(sin(uTime));
            // }else{
            //   _st.y -= (step(1.0, mod(_st.x,2.0)) * 2.0 - 1.0) * abs(sin(uTime));
            // }
            // return fract(_st);
        }
        ...
        void main() {
            ...
            vec3 color = vec3(0.0);
            st = spaceWrap(10.0,10.0,st);

            float boxBorder = box(st,vec2(0.99),0.01);
            // color = vec3(circle(st,0.5));
            color = vec3(boxBorder);
