    ...
    varying vec3 vMp;
      ...
      #define PI2 6.28318530718
        ...
        void main() {
            ...
            //distance()函数，这个函数其实内部调用 length()函数,计算不同两点的距离(在此例中是像素坐标和画布中心的距离)。length()函数内部只不过是用平方根(sqrt())计算斜边的方程。
            vec3 color = vec3(1.0);
            float angle = atan(toCenter.y, toCenter.x);
            float colorRadius = length(toCenter) * 2.0;
            color = hsb2rgb(vec3((angle/ PI2) + radius, colorRadius, 1.0));
            float opacity = 1.0;
            if(pct > radius){
                opacity = 0.0;
            }
